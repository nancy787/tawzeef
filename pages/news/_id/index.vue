    ...
    <section class="my-12">
      <v-container>
        <v-row>
          ...
          <template v-else>
            <v-col cols="12">
              <v-card class="blog-card blog-card--news bordered radius-10">
                <section class="img-wrap ratio-30">
                  <div class="img-content radius-10">
                    <img
                      :src="news.media || require('@/assets/images/avatar.png')"
                      alt=""
                    />
                  </div>
                </section>
            ...
            <v-col cols="12">
              <p class="l-h-2" v-html="news.note[$i18n.locale] || '--'"></p>
            </v-col>
            <v-col cols="12">
...
export default {
  ...
  methods: {
    getBlog() {
      this.loading = true
      this.$axios
        .get(`/visitor/news/${this.$route.params.id}`)
        .then((res) => {
          const { data } = res.data
          this.news = data
          this.loading = false
    ...
    getBlogComments() {
      ...
      this.$axios
        .get(`/visitor/news/${this.$route.params.id}/comments`)
        .then((res) => {
          const { data } = res.data
          this.comments = data
    ...
    sendComment() {
      if (this.$auth.loggedIn) {
        if (this.form.comment) {
          ...
          this.$axios
            .post(`/visitor/news/${this.$route.params.id}/comment`, {
